...
public static class EnumerableExtensions
{
    ...
    public static IEnumerable<T2> Choose<T, T2>(this IEnumerable<T> source, Func<T, Option<T2>> selector) =>
        source.Select(selector)
              ...
              {
                  Some<T2> { Value: var value } => value,
                  _ => throw new UnreachableException("All options should be in the 'Some' state.")
              });
    ...
    public static void Iter<T>(this IEnumerable<T> source, Action<T> action, CancellationToken cancellationToken = default) =>
        source.IterParallel(action, maxDegreeOfParallelism: Option.Some(1), cancellationToken);
    ...
    public static ValueTask IterTask<T>(this IEnumerable<T> source, Func<T, ValueTask> action, CancellationToken cancellationToken = default) =>
        source.IterTaskParallel(action, maxDegreeOfParallelism: Option.Some(1), cancellationToken);
...
public static class AsyncEnumerableExtensions
{
    ...
    /// </summary>
    /// <returns>The first Some value, or <see cref="common.None"/> if no element produces Some.</returns>
    public static async ValueTask<Option<T2>> Pick<T, T2>(this IAsyncEnumerable<T> source, Func<T, ValueTask<Option<T2>>> selector, CancellationToken cancellationToken) =>
        await source.Select(async (T t, CancellationToken _) => await selector(t))
